...
def searchData(StdID='', Firstname='',  Surname='', Age='', Gender='', Location='',  Instrument=''):
    con=sqlite3.connect('Participant.db')
    cur=con.cursor()
    cur.execute('SELECT * FROM Participant WHERE StdID=?, OR Firstname=?, OR Surname=?, OR Age=?, OR Gender=?, OR Location=?, OR Instrument=?', (StdID, Firstname,  Surname, Age, Gender, Location,  Instrument))
    rows=cur.fetchall()
    con.close()
    return rows
